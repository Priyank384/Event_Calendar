...
const Calendar = () => {
  ...
  const [startHour, setStartHour] = useState({ hour: 8, minute: 0 });
  const [endHour, setEndHour] = useState({ hour: 23, minute: 0 });
  ...
  const handleTimeZoneChange = (e) => {
    ...
    if (currentTimeZone === "UTC+0") {
      setStartHour({ hour: 8, minute: 0 });
      setEndHour({ hour: 23, minute: 0 });
    } else if (currentTimeZone === "UTC-2") {
      setStartHour({hour: 6, minute: 0});
      setEndHour({hour: 21, minute: 0});
    } else if (currentTimeZone === "UTC+5:30") {
      setStartHour({ hour: 13, minute: 30 });
    setEndHour({ hour: 13 + 15, minute: 30 });
    }
  };
  ...
  const generateTimeSlots = () => {
    const timeSlots = [];
    let currentTime = moment().set({ hour: startHour.hour, minute: startHour.minute, second: 0 });
    const endTime = moment().set({ hour: endHour.hour, minute: endHour.minute, second: 0 });
